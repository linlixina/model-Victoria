#-----------------------------------------------------
# A comparison of PAR(0) estimators for R0 = 1.5 and 4.
#-----------------------------------------------------
#R0=1.5
library(ggplot2)
library(viridis)
...
theory_df <- data.frame(v = v_vals) %>%
  mutate(
    ...
    PAR_theory = infect_rate_U
  )

# Subset for Stan
subset_df <- theory_df %>% filter(v >= 0.4 & v <= 0.6)
wide_df <- subset_df %>% rename(PAR_R0_1.5 = infect_rate_U)
...
ggsave("figures/Fig3.svg", combined, width = 12.6, height = 4.2, dpi = 600)
