...
y_label_settings <- setNames(
  list(
    expression(bold(PAR['<12y'])),
    expression(bold(PAR['12-15y'])),
    expression(bold(PAR['16-49y, V'])),
    expression(bold(PAR['16-49y, U'])),
    expression(bold(PAR['50+y, V'])),
    expression(bold(PAR['50+y, U']))
  ),
  columns
)
...
plot_list <- lapply(seq_along(ordered_levels), function(i) {
  ...
  p <- ggplot() +
    ...
    geom_ribbon(data = df_fit, aes(x = x, ymin = lower, ymax = upper, fill = "PAR (95% BCI)"),
                alpha = 0.3) +
    geom_line(data = df_fit, aes(x = x, y = y_fit, linetype = "PAR (mean)"),
    ...
    scale_linetype_manual(NULL, values = c("PAR (mean)" = 1)) +
...
print(final_plot)
ggsave("figures/FigS6.svg", plot = final_plot,
       width = 10, height = 6.5, dpi = 600)



#PAR plots with SEX RATIO
fit_lines_all <- data.frame()
points_all <- data.frame()
...
print(final_plot)
ggsave("figures/FigS7.svg", plot = final_plot,
       width = 10, height = 6.5, dpi = 600)
