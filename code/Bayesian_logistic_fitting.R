...
#-------------------------------------------------
# plotting PAR fitted curve using posterior results
#-------------------------------------------------
...
y_label_settings_pcr <- list(
  "Rate(Age<12)"     = expression(bold(PAR['<12y'])),
  "Rate(12<=Age<16)" = expression(bold(PAR['12-15y'])),
  "Rate(16=<V<50)"   = expression(bold(PAR['16-49y, V'])),
  "Rate(16=<UV<50)"  = expression(bold(PAR['16-49y, U'])),
  "Rate(V>=50)"      = expression(bold(PAR['50+y, V'])),
  "Rate(UV>=50)"     = expression(bold(PAR['50+y, U']))
)
...
plot_list <- lapply(seq_along(ordered_levels), function(i) {
  ...
  p <- ggplot() +
    ...
    geom_ribbon(
      data = df_fit,
      aes(x = x, ymin = lower, ymax = upper, fill = "PAR (95% BCI)"),
      alpha = 0.3
    ) +
    geom_line(
      data = df_fit,
      aes(x = x, y = y_fit, color = "PAR (mean)"),
      linewidth = 1
    ) +
    geom_point(
      data = df_pts,
    ...
    scale_color_manual(NULL, values = c("PAR (mean)" = "black")) +
    scale_fill_manual(NULL, values = c("PAR (95% BCI)" = "#B0C4DE")) +
...
#-----------------------------------------------------------------
# plotting PAR (v from 0 to 1) fitted curve using posterior results
#-----------------------------------------------------------------
