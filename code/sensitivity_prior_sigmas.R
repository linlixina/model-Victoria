...
for (sigma in sigma1_list) {
  ...
  for (grp in groups_code1) {
    ...
    sumdf <- summary(fit, pars = c("L", "PAR0"))$summary %>%
      as.data.frame() %>%
      rownames_to_column("parameter") %>%
      filter(parameter %in% c("L", "PAR0")) %>%
      mutate(group          = grp,
             prior_mu_L     = 0.01,
             prior_sigma_L  = sigma)
...
group_labels <- c(
  ...
  "Rate(16=<V<50)"   = "16–49 years, V",
  "Rate(16=<UV<50)"  = "16–49 years, U",
  "Rate(V>=50)"      = "50+ years, V",
  "Rate(UV>=50)"     = "50+ years, U"
)
