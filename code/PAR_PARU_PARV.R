...
df <- expand.grid(v = v_vals, R0 = R0_list) %>%
  ...
  mutate(
    ...
    PAR_pp     = Zpp / (1 - p),
    PAR_U_pp   = R0 * e1 * (Zpp - R0 * Zp^2),
    PAR_V_pp   = (1 - k)*R0 * e2 * (Zpp - (1 - k)*R0 * Zp^2)
  )

v_star <- df %>%
  group_by(R0) %>%
  summarise(
    v_star_PAR   = find_roots(v, PAR_pp),
    v_star_PAR_U = find_roots(v, PAR_U_pp),
    v_star_PAR_V = find_roots(v, PAR_V_pp)
  ) %>%
  ungroup() %>%
  pivot_longer(starts_with("v_star"), names_to = "nm", values_to = "vstar") %>%
  mutate(
    Function = recode(nm,
                      v_star_PAR   = "bold(PAR(v))",
                      v_star_PAR_U = "bold(PAR[U](v))",
                      v_star_PAR_V = "bold(PAR[V](v))"),
    Order = "2nd"
  )
...
df_long <- df %>%
  select(v, R0,
         PAR,   PAR_p,   PAR_pp,
         PAR_U, PAR_U_p, PAR_U_pp,
         PAR_V, PAR_V_p, PAR_V_pp) %>%
  pivot_longer(-c(v, R0), names_to = "nm", values_to = "value") %>%
  mutate(
    Function = case_when(
      grepl("^PAR_V", nm) ~ "bold(PAR[V](v))",
      grepl("^PAR_U", nm) ~ "bold(PAR[U](v))",
      TRUE                ~ "bold(PAR(v))"
    ),
    Order = case_when(
      grepl("_pp$", nm) ~ "2nd",
...
ann <- expand.grid(
  Order    = c("0th","1st","2nd"),
  Function = c("bold(PAR(v))","bold(PAR[U](v))","bold(PAR[V](v))")
) %>%
...
ggsave("figures/FigS9.svg", p_plot,
